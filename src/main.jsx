 ...
 return <main className="app">
  ...
  <section>
   ...
   {logs.map(x=><article className="log" key={x.date}>
    <h3>{x.date} · {x.part}</h3><pre>{x.content}</pre><p>{x.note}</p>
    <div className="photo">📷 训练照片记录</div>
   </article>)}
  </section>
  <section>
   <h2>🏆 Milestones</h2>
   <article className="milestone"><b>2026.09.05 卧推突破80kg</b><p>经过持续训练，实现阶段性力量突破。</p></article>
  </section>
  <section className="ai-box">
   <h2>🤖 FitBuddy AI</h2>
   <p>输入训练记录，让 AI 自动整理并沉淀到腾讯文档。</p>
   <div className="example">今天练胸，上斜30kg三组，卧推60/70/80kg，帮我记录。</div>
   <button>✨ 使用 WorkBuddy 整理训练记录</button>
  </section>
  <section className="doc-box">
   <h2>📄 FitBuddy 成长档案</h2>
   <p>腾讯文档作为训练日志、里程碑和成长总结的长期沉淀空间。</p>
